...
//处理授信
func (this *WorkspaceController) LinkManQueue() {
	isPermission := models.IsDataPermissionByStationId(this.User.StationId, 7)
	if !isPermission {
		this.Abort("没有该数据权限!")
	}
	this.IsNeedTemplate()
	...
	defer func() {
		...
		if okuser.Flag {
			...
			this.TplName = "workspace/credit_linkman.html"
			return
		}
	}()

	//处理异常关闭
	okuser.Id, okuser.Uid, okuser.TimeDff, okuser.Flag = check_linkman_credit_handing(this)
	//45分钟之内 handing状态
	if okuser.Flag {
		return
	}
	...
	if this.User.LoginState == "OFFLINE" && !okuser.Flag {
		...
		cache.RecordLogs(this.User.Id, 0, this.User.Name, this.User.DisplayName, "没有订单", "workspace/linkmanqueue", "", this.Ctx.Input)
		this.TplName = "workspace/credit_linkman.html"
		return
	}
	...
	go credit_linkman_queue_a(pt, this)
	pt_tmp = <-pt
	if !pt_tmp.Flag {
		go credit_linkman_queue_p(pp, this)
		pp_tmp = <-pp
	}
...
func credit_linkman_queue_a(pt chan utils.ShareDdata, this *WorkspaceController) {
	//统计预约插队数量
	count := models.GetLinkManNumCreditQueueAp()
	go func() {
		for {
			ca, _ := models.GetLinkManAppointmentQueueId()
			if ca.InqueueType == 1 {
				//lock false 为lock状态 ture 为unlock状态
				_, flag := cache.GetLinkManCacheCreditMessage(ca.CreditAduitId)
				if flag {
					//通道放入值
					pt <- utils.ShareDdata{Id: ca.CreditAduitId, Uid: ca.Uid, Flag: true, TimeDff: 45 * 60}
					credit_linkman_queue_change_state(this, ca.CreditAduitId, ca.Uid)
					return
				}
			}
			//退回队列中有排队 放入队列中
			if ca.InqueueType == 2 {
				err := models.UpadateLinkManInqueueType(ca.Id, ca.InqueueTime)
				if err != nil {
...
func credit_linkman_queue_p(pp chan utils.ShareDdata, this *WorkspaceController) {
	//后期考虑使用 select case 加上timeout机制
	//统计排队中数量
	count := models.GetCreditLinkManQueueUpIdCount()
	go func() {
		for {
			cp, _ := models.GetCreditLinkManQueueUpId()
			//排队中有满足订单情况
			if cp.Uid != 0 {
				_, flag := cache.GetLinkManCacheCreditMessage(cp.CreditAduitId)
				if flag {
					pp <- utils.ShareDdata{Uid: cp.Uid, Id: cp.CreditAduitId, Flag: true, TimeDff: 45 * 60}
					credit_linkman_queue_change_state(this, cp.CreditAduitId, cp.Uid)
					return
				}
			}
			//排队中没有满足订单的情况 防止死锁
...
func check_linkman_credit_handing(this *WorkspaceController) (id, uid int, timediff float64, flag bool) {
	models.CreditLinkManHandingLogOut() //睡眠 关机
	id = cache.GetLinkManCacheCreditHandingUids(this.User.Id)
	uid, flag = cache.GetLinkManCacheCreditMessage(id)
	if !flag {
		oc := models.QueryLinkManCreditAttime(id)
		timediff := oc.AllocationTime.Add(45 * time.Minute).Sub(time.Now()).Seconds()
		if timediff <= 0 {
			credit_linkman_time_out_handle(this, id, uid)
			return 0, 0, 0, false
		} else {
...
func credit_linkman_time_out_handle(this *WorkspaceController, credit_aduit_id, uid int) {

	//45分钟之外 超时
	err := models.UpdateLinkManCreditQueueing(credit_aduit_id)
	if err != nil {
	...
	err = utils.Rc.Delete("linkman:" + utils.CacheKeyCreditMessage + "_" + strconv.Itoa(credit_aduit_id))
	if err != nil {
		cache.RecordLogs(this.User.Id, 0, this.User.Name, this.User.DisplayName, "删去redis失败", "workspace/creditqueuehanding", err.Error(), this.Ctx.Input)
	}
	//增加超时记录
...
//授信状态redis改变
func credit_linkman_queue_change_state(this *WorkspaceController, credit_aduit_id, uid int) {

	//存redis
	if utils.Re == nil {
		err := utils.Rc.Put("linkman:"+utils.CacheKeyCreditHandingUids+"_"+strconv.Itoa(this.User.Id), credit_aduit_id, utils.RedisCacheTime_Year)
		if err != nil {
		...
		err = utils.Rc.Put("linkman:"+utils.CacheKeyCreditMessage+"_"+strconv.Itoa(credit_aduit_id), uid, utils.RedisCacheTime_Year)
		if err != nil {
			cache.RecordLogs(this.User.Id, 0, this.User.Name, this.User.DisplayName, "存入redis失败", "workspace/creditqueuehanding", err.Error(), this.Ctx.Input)
			return
		}
	...
	//分配成功 "handing"
	err := models.UpdatLinkManCueditQueueStatusInqueueTime("HANDING", this.User.DisplayName, this.User.Id, credit_aduit_id)
	if err != nil {
		cache.RecordLogs(this.User.Id, 0, this.User.Name, this.User.DisplayName, "更新状态失败", "workspace/creditqueuehanding", err.Error(), this.Ctx.Input)
	}
	//更新主表状态
	...
	//增加预防超时
	err = models.UpdateLinkManCreditAlloctionTime(credit_aduit_id)
	if err != nil {
