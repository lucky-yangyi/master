...
type SysStation struct {
	...
	IsCollectStation int    `orm:"column(is_collect_station)"` //是否是催收岗位：0-不是 1-是
}
...
func GetDataByStation(stationId int) (str string, err error) {
	var dataList []*SysStationData
	sql := `SELECT org_id FROM sys_station_data WHERE station_id=? GROUP BY org_id ORDER BY org_id ASC `
	_, err = orm.NewOrm().Raw(sql, stationId).QueryRows(&dataList)
	...
	utils.Rc.Put(utils.CacheKeySysStationData+strconv.Itoa(stationId), str, 5*time.Minute)
	return str, err
}

func GetOrgId(staId int) (org_id int, err error) {
	sql := `select org_id from sys_station where id = ?`
	err = orm.NewOrm().Raw(sql, staId).QueryRow(&org_id)
	return
}

func GetStation(staId int) (stationName string, err error) {
	sql := `select name from sys_station where id = ?`
	err = orm.NewOrm().Raw(sql, staId).QueryRow(&stationName)
	return
}
